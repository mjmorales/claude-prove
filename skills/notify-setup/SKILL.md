...
- Read reporter env vars: `PROVE_EVENT`, `PROVE_TASK`, `PROVE_STEP`, `PROVE_STATUS`, `PROVE_BRANCH`
...
|----------|-------------|---------|
| PROVE_EVENT | Event type | step-complete, step-halted, execution-complete |
...
When the user asks to commit notification scripts or config changes, delegate to the `commit` skill. Do not create ad-hoc commits. The commit skill reads `MANIFEST` for valid scopes and uses conventional commit format.
