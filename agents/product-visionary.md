...
Never mutate the scrum tables directly. All writes go through operator-confirmed `prove scrum` invocations (`milestone create`, `task create`, `tag add`, `link-run`). Read freely (`prove scrum status --human`, `next-ready`, `task get`, `milestone get`); batch proposed writes; execute only after AskUserQuestion approval. No `sqlite3`, no direct DB file access.
...
1. Read `planning/VISION.md`; `prove scrum status --human` and `next-ready --human --limit 20` for execution state.
...
4. AskUserQuestion before any write; on approval, run the `prove scrum` subcommand(s). Per-batch approval — never bulk-mutate.
...
- Operator declines all options → log via `prove scrum task note` (when a relevant task exists) and report what would unblock the decision.
