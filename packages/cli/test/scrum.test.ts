...
describe('prove scrum hook', () => {
  ...
  test('valid JSON with Task-4-pending handlers: exit 1 with not-yet-available', () => {
    // This test documents the Task 5 <-> Task 4 coupling: until Task 4's
    // hook.ts lands, any real payload surfaces the stub's throw. The
    // orchestrator merge replaces hook.ts and this test should start
    // asserting real dispatch behavior.
    const repo = trackRepo('hook-pending');
    const res = runScrum(['hook', 'stop'], repo, JSON.stringify({ event: 'Stop' }));
    expect(res.exitCode).toBe(1);
    expect(res.stderr).toContain('not yet available');
  });
});
