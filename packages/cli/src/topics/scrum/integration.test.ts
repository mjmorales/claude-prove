...
describe('scrum full lifecycle', () => {
  test('create -> tag -> dep -> events -> link -> bundle -> nextReady -> delete', async () => {
    // 1. Milestones + tasks
    await store.createMilestone({ id: 'phase-12', title: 'Phase 12', status: 'active' });
    ...
    const reconciler = await store.createTask({
      id: 'scrum-2',
      title: 'Reconciler',
      milestoneId: 'phase-12',
      tags: ['p1'],
    });
    const ui = await store.createTask({
      id: 'scrum-3',
      title: 'UI',
      milestoneId: 'phase-12',
    });
    ...
    expect(await store.listTasks({ milestoneId: 'phase-12' })).toHaveLength(3);

    // 2. Tags — verify addTag + listTagsForTask + listTasksForTag
    await store.addTag('scrum-2', 'p0');
    expect((await store.listTagsForTask('scrum-2')).map((t) => t.tag).sort()).toEqual(['p0', 'p1']);
    expect((await store.listTasksForTag('p0')).map((t) => t.id).sort()).toEqual([
      'scrum-1',
      'scrum-2',
    ]);

    // 3. Dependencies — scrum-1 blocks scrum-2, scrum-2 blocks scrum-3
    await store.addDep('scrum-1', 'scrum-2', 'blocks');
    await store.addDep('scrum-2', 'scrum-3', 'blocks');
    expect((await store.getBlocking('scrum-1')).map((d) => d.to_task_id)).toEqual(['scrum-2']);
    expect((await store.getBlockedBy('scrum-3')).map((d) => d.from_task_id)).toEqual(['scrum-2']);
    ...
    const events = await store.listEventsForTask('scrum-1');
    // 1 seed `task_created` + 2 appended = 3
    expect(events).toHaveLength(3);
    expect(events[0]?.kind).toBe('steward_verdict'); // newest-first

    const reloaded = await store.getTask('scrum-1');
    ...
    const found = await store.getTaskForRun('.prove/runs/feat/phase-12/scrum-1');
    expect(found?.id).toBe('scrum-1');
    expect(await store.listRunsForTask('scrum-1')).toHaveLength(1);

    // 6. Context bundle — save, reload, upsert, reload
    await store.saveContextBundle('scrum-1', {
      relevantFiles: ['packages/cli/src/topics/scrum/store.ts'],
    });
    const bundle1 = await store.loadContextBundle('scrum-1');
    expect(bundle1?.bundle).toEqual({ relevantFiles: ['packages/cli/src/topics/scrum/store.ts'] });

    await store.saveContextBundle('scrum-1', { relevantFiles: ['updated.ts'] });
    const bundle2 = await store.loadContextBundle('scrum-1');
    ...
    const ranked = await store.nextReady({ milestoneId: 'phase-12' });
    ...
    await store.updateTaskStatus('scrum-1', 'review');
    await store.updateTaskStatus('scrum-1', 'done');
    const final = await store.getTask('scrum-1');
    expect(final?.status).toBe('done');

    // 9. Soft-delete — scrum-3 vanishes from listTasks but events remain
    await store.softDeleteTask('scrum-3');
    expect(await store.getTask('scrum-3')).toBeNull();
    expect((await store.listTasks({ milestoneId: 'phase-12' })).map((t) => t.id).sort()).toEqual([
      'scrum-1',
      'scrum-2',
    ]);
    // Reconciler is still visible — regression for the filter predicate.
    expect((await store.getTask('scrum-2'))?.status).toBe('backlog');

    // 10. Milestone close — also stamps closed_at
    const closed = await store.closeMilestone('phase-12');
  ...
  test('store survives reopening the same :memory: path via separate handles', async () => {
    ...
    const s1 = await openScrumStore({ path: ':memory:' });
    try {
      await s1.createTask({ id: 't1', title: 'Test' });
    } finally {
      s1.close();
    }
    const s2 = await openScrumStore({ path: ':memory:' });
    try {
      expect(await s2.getTask('t1')).toBeNull();
    } finally {
      s2.close();
    }
  });
