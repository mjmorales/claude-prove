...
describe('resolveBinaryPath', () => {
  ...
  test('compiled mode defaults to $HOME/.local/bin/prove', () => {
    const expected = join(homedir(), '.local', 'bin', 'prove');
    expect(resolveBinaryPath('compiled', {})).toBe(expected);
  });

  test('compiled mode honors an explicit binaryPath override', () => {
    expect(resolveBinaryPath('compiled', { binaryPath: '/opt/prove/bin/prove' })).toBe(
      '/opt/prove/bin/prove',
    );
  });
