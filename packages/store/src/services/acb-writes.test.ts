...
async function rowsFor(store: Store, slug: string): Promise<GroupVerdictRecord[]> {
...
describe('upsertGroupVerdict', () => {
  test('a single upsert lands exactly one row with the returned shape', async () => {
    const store = await makeStore();
    try {
      const record = await upsertGroupVerdict(
        store,
        'add-login',
        'g1',
        'accepted',
      ...
      const rows = await rowsFor(store, 'add-login');
  ...
  test('a re-upsert on the same (slug, groupId) updates in place', async () => {
    const store = await makeStore();
    try {
      const first = await upsertGroupVerdict(store, 'add-login', 'g1', 'pending', null, null);

      // Force a distinct ISO timestamp so the bump is observable; the stamp is
      // millisecond-resolution and a back-to-back re-upsert can collide otherwise.
      ...
      try {
        second = await upsertGroupVerdict(
          store,
          'add-login',
          'g1',
          'rework',
