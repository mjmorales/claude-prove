...
1. **External references**: If `claude_md.references` is absent or empty, scan `$PLUGIN_DIR/references/` for bundled `.md` files. Extract labels from first `# Heading` line (fall back to filename sans extension). **Exclude `claude-prove-reference.md`, `design-principles.md`, and `agent-routing.md`** — the composer injects all three as built-in defaults; offering them here would create duplicates the dedup logic silently drops. **Also exclude `planning-mode-routing.md`** — a skill-internal procedure the planning skills load directly, not a CLAUDE.md import candidate.
...
3. **Optional config blocks** (config schema v9/v10/v13/v14): the `.claude/.prove.json` schema gains optional config blocks. The version stamp is migrated in by Step 4's `schema migrate` — there is nothing to add by hand — but surface the tunables so the operator knows they exist:
   ...
   - `models.planning` (v14; absent resolves as `auto`) — planning-phase routing (`prove|native|auto`): `native` runs `/prove:plan` Mode: Task and orchestrator full-mode PRD gathering inside Claude Code plan mode (where an `opusplan` main gets its Opus upgrade), `prove` keeps the prove-specific flows, `auto` resolves native iff the declared `main` is an opusplan alias. Prove-side only — never materialized; `claude-prove models routing` prints the resolved route.

   ```
   New plugin feature: optional config blocks (schema v9/v10/v13/v14)
...
4. **New methodology skills + CLI surfaces**: these are discovered automatically on plugin load (skills/agents) or ship in the CLI — there is no `.claude/.prove.json` change. Note them so the operator knows the capabilities are live:
   ...
   - **`scrum milestone update <id> [--title T] [--description D] [--target-state S] [--initiative I]`** — amend a milestone's descriptive fields after creation (only the passed flags write; an empty value clears a nullable field; a closed milestone is rejected). **`scrum annotation add|list --target-kind milestone`** — attach append-only notes to a milestone (soft reference; joins the closed `task|team|decision|milestone` set). No DB migration: the `target_kind` column has no CHECK, so both are live on existing stores immediately.
   ...
   - **`models` topic (schema v13)** — **`models set [--preset <name>] [--main M] [--advisor M] [--fallback csv] [--effort L] [--planning prove|native|auto]`** declares the committed model recommendation (`--preset` replaces the block with a use-case-tuned pairing from the closed table listed by **`models presets`**: `balanced`, `deep`, `economy`, `unattended`), **`models apply [--dry-run]`** materializes it into the gitignored `.claude/settings.local.json` per machine, **`models status`** reports declared-vs-materialized state with pairing diagnostics, and `install doctor` gains a warn-only `models-drift` check naming `models apply` as the repair. **`/prove:models`** + the **`model-config`** skill + the **`model-config-advisor`** agent drive the judgment flow: workload gathering, an agent recommendation, and human gates on both the declaration and the materialization. **`models routing`** prints the resolved planning route (`native`|`prove`) — the one-word contract `/prove:plan` and orchestrator full mode branch on, with the **`plan-backfiller`** agent transforming an approved plan-mode plan back into prove artifacts on the native route.
