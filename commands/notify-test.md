...
Use the plugin's test script at `scripts/notify-test.sh` to execute the test:
```bash
bash "$PLUGIN_DIR/scripts/notify-test.sh" [event-type]
```

Note: Reporter scripts generated by `/prove:notify-setup` are placed in `.prove/` (project scope) or `~/.claude/scripts/` (global scope), not in the plugin's `scripts/` directory.
