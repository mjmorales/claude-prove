...
1. **`claude-prove` installed**: `command -v claude-prove >/dev/null` — if missing, print: "`claude-prove` CLI not found on PATH. Install via \`curl -fsSL https://raw.githubusercontent.com/mjmorales/claude-prove/main/scripts/install.sh | bash\`. Dev-mode users: alias or symlink `claude-prove` to your working-tree entry point." Stop.
